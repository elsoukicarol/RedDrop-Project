"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CharitiesService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const charity_entity_1 = require("./entities/charity.entity");
const typeorm_2 = require("typeorm");
let CharitiesService = class CharitiesService {
    constructor(charityRepository) {
        this.charityRepository = charityRepository;
    }
    async create(createCharityDto) {
        const newCharity = this.charityRepository.create(createCharityDto);
        await this.charityRepository.save(newCharity);
        return newCharity;
    }
    async deleteCharity(id) {
        const result = await this.charityRepository.delete(id);
        if (result.affected === 0) {
            throw new common_1.NotFoundException(`Charity with ID "${id}" not found.`);
        }
        return { message: 'Charity deleted successfully' };
    }
    async updateCharity(id, updateCharityDto) {
        const charity = await this.charityRepository.findOneBy({ id });
        if (!charity) {
            throw new common_1.NotFoundException(`Charity with ID "${id}" not found.`);
        }
        const updatedCharity = this.charityRepository.merge(charity, updateCharityDto);
        await this.charityRepository.save(updatedCharity);
        return updatedCharity;
    }
    async getCharity(id) {
        const charity = await this.charityRepository.findBy({ id: id });
        if (charity.length === 0) {
            throw new common_1.NotFoundException(`Charity with ID "${id}" not found.`);
        }
        return charity;
    }
    async getAllCharities() {
        const charities = this.charityRepository.find();
        if (!charities) {
            throw new common_1.NotFoundException(`No charities found`);
        }
        return charities;
    }
    findAll() {
        return `This action returns all charities`;
    }
    findOne(id) {
        return `This action returns a #${id} charity`;
    }
    update(id, updateCharityDto) {
        return `This action updates a #${id} charity`;
    }
    remove(id) {
        return `This action removes a #${id} charity`;
    }
};
exports.CharitiesService = CharitiesService;
exports.CharitiesService = CharitiesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(charity_entity_1.Charity)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], CharitiesService);
//# sourceMappingURL=charities.service.js.map